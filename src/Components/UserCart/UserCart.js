               ...
               {!IsLoading?
                    <div className={`${styles.cardFig}`}>
                        ...
                        <div className={`${styles.cardLink}`}>
                            <Link to={`/userCase/${props.id}`} className={`${styles.cardDonate}`}>{t("للمزيد من التفاصيل ")} </Link>
                        </div>
                ...
                <>
                   {!IsLoading?  <h3 className={`${styles.cardContentTitle}`}>{props.title}</h3> : <Skeleton  className={`skeleton-title`} /> }
                    ...
                    {!IsLoading? 
                       ...
                       {!IsLoading? 
                       <>
                         {props.status === "rejected" ?  <p className='mt-2'>  سبب الرفض: {props.reason_reject_ar}</p>:null}
